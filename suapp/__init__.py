...
class SuApp(object):
    ...
    def configure_log(self):
        ...
        # Logging not configured yet, so postponing log.debug(">do_log_configuration()")
        try:
            ...
            log.debug(">do_log_configuration() [DELAYED]")
        ...
        log.debug("<do_log_configuration() %s" % (self.configuration["log"]))
    
    @loguse
    def import_target(self):
        if not "target" in self.configuration:
            self.configuration["target"] = "test"
    ...
    def read_flow(self, flowfile):
        ...
        log.debug("!read_flow IP: %s" % installpath)
        log.debug("!read_flow CP: %s" % configpath)
        try:
...
class SuApp(object):
    ...
    def read_flow(self, flowfile):
        ...
        # 2. Look starting from the directory of the config file (if different).
        configpath = os.path.dirname(self.configuration["self"])
        print("IP: %s" % installpath)
        print("CP: %s" % configpath)
        try:
            with open(os.path.join(installpath, flowfile)) as fh:
                for line in fh:
                    (out, intag, objecttype) = self.parse_flow_line(line)
