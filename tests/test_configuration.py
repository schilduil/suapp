...
sample = {
    ...
    "log": {
        ...
        "modules": {
            "base": {"level": "INFO"},
            "httpd": {"level": "DEBUG", "filename": "~/.suapp/log/httpd.accces_log"},
        },
    },
    "datasource": {"type": "Plank", "location": "~/.susm/data/"},
    "modules": {"base": {}},
}
...
def test_missing_json_with_backup(json_file):
    ...
    """
    with suapp.configuration.get_configuration(
        [json_file, url, copy.deepcopy(sample)]
    ) as test:
        assert test == sample
...
def test_missing_cfg_file_with_backup(cfg_file):
    ...
    with suapp.configuration.get_configuration(
        [cfg_file, url, copy.deepcopy(sample)]
    ) as test:
        assert test == sample
...
def test_different_cfg_with_backup(cfg_file):
    ...
    with suapp.configuration.get_configuration(
        [cfg_file, copy.deepcopy(different_sample)]
    ) as test:
        test.save()
...
def test_missing_xml_with_backup(xml_file):
    ...
    with suapp.configuration.get_configuration(
        [xml_file, url, copy.deepcopy(sample)]
    ) as test:
        assert test == sample
...
def test_different_xml_with_backup(xml_file):
    ...
    with suapp.configuration.get_configuration(
        [xml_file, copy.deepcopy(different_sample)]
    ) as test:
        test.save()
...
def test_missing_yaml_with_backup(yaml_file):
    ...
    with suapp.configuration.get_configuration(
        [yaml_file, url, copy.deepcopy(sample)]
    ) as test:
        assert test == sample
...
def test_different_yaml_with_backup(yaml_file):
    ...
    with suapp.configuration.get_configuration(
        [yaml_file, copy.deepcopy(different_sample)]
    ) as test:
        test.save()
    print("=== START FILE CONTENT ===")
...
def test_ConfigurationParser():
    ...
    assert (
        string
        == "<suapp.configuration.ConfigurationParser object at 0x000000000000> on location here"
    )
...
def test_sub_ConfigurationParser(empty_subclass_ConfigurationParser):
    ...
    assert (
        string
        == "<test_configuration.empty_subclass_ConfigurationParser.<locals>.SubConfigurationParser object at 0x000000000000> on location here"
    )
